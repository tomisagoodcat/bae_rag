...
- Step 1: Extract all candidate `mp:Statement` nodes from the text.  
- Step 2: From the candidate mp:Statementnodes, determine which ones represent central scientific assertions — specifically, those aligned with the WHU_DATASET criteria and associated with either MP_SUPPORT or MP_STATEMENT.→ Label these nodes as mp:Claim.
- Step 3: Keep the remaining ones as general `mp:Statement`.  
- Always maintain the correct semantic links:  
  * `whu:DataSet` → `mp:supports` / `mp:challenges` → `mp:Claim`  
  * `mp:Statement` → `mp:supports` / `mp:challenges` → `mp:Claim`  

 

#### 2. Experiment and activity Rule
- The Experime(bio_experiment and )
...
#### 1.1 entitiy:"WHU_BIO_CHEMICAL_EXPERIMENT","WHU_BIOCHEMICALACTIVITYSTEP", "MP_CLAIM","WHU:DATASET"
#### 1.2 relation:  
* [ "WHU_BIOCHEMICALACTIVITYSTEP","P_PLAN_ISSTEPOFPLAN","WHU_BIO_CHEMICAL_EXPERIMENT"]
