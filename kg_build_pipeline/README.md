...
| `build_kg` | 按 `potential_schema` 逐条 LLM 抽取（可用 `schema_tiers: [mid]`） | 1_2_0_2 模块3 | `stages.build_kg.run_build_kg` |
| `mid_quality_gate` | Mid 校验 + Qwen Review + 定向重抽；写回 `mid_gate_status` | Mid 分层 | `stages.mid_quality_gate.run_mid_quality_gate` |
| `low_expand` | 对 PASS 文档做 parent-scoped mid2low/low 展开 + Low SHACL/repair + cross-parent | Low 分层 | `stages.low_expand.run_low_expand` |
...
UI 抽取模式：`Extract Mid` / `Extract Low`（`expand_mid`，依赖已有 PASS） / `Extract Mid + Low` / 废弃的 `Extract Low and All`（与 `low_expand` 隔离）。配置块见 `low_extraction`（`strategy=entity_first|attach_first`，`routing.mode=incident_two_wave|legacy_closure`，`extract.batch_schemas_per_call`，`abort_on_insufficient_balance`）。
...
2. 设置环境变量（或在父目录 `cursorParperExtarct/.env` 中配置，见 `kg_build_pipeline/.env.example`）：
   - `NEO4J_PASSWORD` — 必需
   - `DEEPSEEK_API_KEY` — `build_kg` / mid 重抽必需
   - `QWEN_API_KEY` — `mid_quality_gate` 必需；MetaPath F2/F3 可选（未设置且 `metapath.skip_f2_without_qwen: true` 时跳过 F2/F3）
...
5. **Python 环境**：`build_kg` 依赖 PyTorch。本机 Windows Store Python + `pip install torch` 会出现 `c10.dll WinError 1114`；请使用 conda 环境 `tomluck2`，或以其为基重建 `pipelineD_env`：
   ```bat
   C:\Users\tom\.conda\envs\tomluck2\python.exe -m venv --system-site-packages pipelineD_env
   pipelineD_env\Scripts\pip install -r kg_build_pipeline\requirements.txt
   ```
   `run_pipeline.bat` / `run_ui.bat` 默认指向 `tomluck2`；若手动启动 UI，请用上述 Python，勿用已损坏的旧 venv 进程。
...
脚本会：切换到仓库根目录 → 使用 conda `tomluck2`（或 `PIPELINE_PYTHON` 覆盖）→ 执行 `run_pipeline.py --all`。
...
| `build_kg.schema_tiers` | `[mid]`（试点） | 仅抽取指定 tier；清空则全量 |
| `mid_quality_gate.max_iterations` | `3` | Mid Review 循环上限 |
| `mid_quality_gate.pass_score` | `0.78` | 与 `hard_count==0` 同时满足才 PASS |
| `mid_quality_gate.pass_on_hard_zero` | `false` | 为 false 时：PASS = hard=0 **且** overall_score ≥ pass_score（与 reviewer 一致；true 则 hard=0 即放行） |
| `mid_quality_gate.reject_before_reextract` | `true` | reextract 前按 validator 报告 mark/delete 问题节点 |
...
| `link_mid_to_low: 未创建任何 hasDetailPath`（low>0） | mid 锚点与 low 实体未共享 `FROM_CHUNK` 的 Chunk | 检查实体 `FROM_CHUNK` 溯源；属 F4 层级边策略（Chunk 共现）。流水线默认 `metapath.acceptance.mode=lenient` 时仅警告不阻断；要硬失败则改 `strict` |
| `N 个 mid 无 hasDetailPath 子路径` | Low 链不完整或 Chunk 未共现，部分 mid 无子路径 | `lenient` 下默认允许；可用 `max_orphan_mid_ratio` 限制 orphan 比例 |
