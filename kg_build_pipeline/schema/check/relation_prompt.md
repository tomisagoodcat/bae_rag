...
When ScienceEvidence mp_supports SupportGraph, the connector must appear in the shared argumentative span. ScienceEvidence must not mp_supports/mp_challenges Claim directly.
...
ScienceEvidence -> DataSet/Method; SupportGraph -> Statement/Attribution/Reference
...
- Focal Claim linkage to SupportGraph uses `mp_supports`/`mp_challenges`, not hadMember

**中文**

- ScienceEvidence → DataSet 和/或 Method：ScienceEvidence 建立所必需
- SupportGraph → Claim：使用 mp_supports/mp_challenges，不用 hadMember
...
Do **not** use SupportGraph -> `prov_hadMember` -> ScienceEvidence or Claim. Link ScienceEvidence to SupportGraph via `mp_supports`/`mp_challenges`; link focal Claim via SupportGraph -> `mp_supports`/`mp_challenges` -> Claim.
...
仅表示成员关系；极性由 mp_supports/mp_challenges 表达，而非 hadMember。禁止 SupportGraph → `prov_hadMember` → ScienceEvidence 或 Claim。
