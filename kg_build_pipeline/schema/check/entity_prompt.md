...
May receive mp_supports/mp_challenges from SupportGraph only (never directly from ScienceEvidence). When co-created with SupportGraph, link via SupportGraph -> mp_supports/mp_challenges -> Claim (not prov_hadMember).
...
4. a co-created `whu_SupportGraph` in the same pass, with ScienceEvidence linked via `mp_supports`/`mp_challenges` to that SupportGraph only (never directly to Claim; not via `prov_hadMember`)
...
4. 同一轮次共创建 `whu_SupportGraph`，且 ScienceEvidence 经 `mp_supports`/`mp_challenges` **仅**链接到该 SupportGraph（**不得**直连 Claim；**不要**用 `prov_hadMember`）
...
“CVAFS measurements showed a 42% decrease in grain MeHg, supporting the biochar hypothesis.” -> ScienceEvidence mp_supports SupportGraph; SupportGraph mp_supports Claim (not ScienceEvidence -> Claim, not hadMember).

**中文**

“CVAFS measurements showed a 42% decrease in grain MeHg, supporting the biochar hypothesis.” → ScienceEvidence mp_supports SupportGraph；SupportGraph mp_supports Claim（禁止 ScienceEvidence→Claim，禁止 hadMember）。
...
SupportGraph -> `mp_supports`/`mp_challenges` -> Claim (required when co-created). ScienceEvidence is **not** a `prov_hadMember` of SupportGraph; attach it with `mp_supports`/`mp_challenges` instead.
...
`prov_hadMember` to Statement/Attribution/Reference only (not Claim). `mp_supports`/`mp_challenges` to focal Claim. ScienceEvidence polarity via `mp_supports`/`mp_challenges` to SupportGraph only (ScienceEvidence -> SupportGraph; never ScienceEvidence -> Claim).

**中文**

`prov_hadMember` 仅链接 Statement/Attribution/Reference（不含 Claim）。`mp_supports`/`mp_challenges` 链接焦点 Claim。ScienceEvidence 极性经 `mp_supports`/`mp_challenges` **仅**连 SupportGraph（禁止 ScienceEvidence→Claim）。
...
Claim about biochar reducing MeHg with ScienceEvidence mp_supports SupportGraph, SupportGraph mp_supports Claim, and cited References as SupportGraph members.

**中文**

关于 biochar 降低 MeHg 的 Claim：ScienceEvidence mp_supports SupportGraph，SupportGraph mp_supports Claim，引用文献作为 SupportGraph 成员。
